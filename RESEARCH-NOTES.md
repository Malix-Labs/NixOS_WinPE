...
**ROUND 21 (2026-09-19, LOCAL REPLICA ONLY — NO USER REBOOT SPENT):**
- Round-20 fix (VC90 CRT SxS store graft + Winners + vendor MFC private-assembly dir) committed locally, winpe-image zfciba6923… REPLICA-VALIDATED: FWUpdLcl (the toolchain's 64-bit Intel tool) **RAN end-to-end** in the replica (printed its full CLI help + "Error 8716") — proof the 32-bit actctx pre-load SxS chain works now. H2OFFT-W (32-bit GUI) still dies -1073741515, BUT with **NO error dialog** (replica screendumps n20d): the failure is now the classic silent dependent-DLL load, NOT the SxS dialog. Round-19's dialog text is GONE — the actctx fix WORKED.
- Round 21 (current, uncommitted): winpe-image grafts extended with the still-missing 32-bit ICON/dlg support DLLs the Insyde GUI shell statically imports and this WinRE base lacks in SysWOW64: **dwmapi.dll / winmm.dll / uxtheme.dll / comdlg32.dll** (all present in ESD image 4 SysWOW64 dir, verified by winimdir 2026-09-19). Image 8pr1nvkqr has all 4.
- DUMP FIRST in a LOADED replica (boot-n21.wim, stage2.sh, autorun-pause.cmd – the pause gives the screendumping loop a 200s window to photograph the dialog box; earlier replica boots lost the deaths to the 4s auto-reboot window) — NOT under wine. Wine's "cannot load driver" halt line is a different layer (kernel driver load), and wine's SxS binder only cares about a fraction of these features: the round-19 SxS actctx dialog NEVER fires under wine.
- Replication recipe (validated now): 1) build rep eval `nix build --no-link --print-out-paths -f /home/malix/Repositories/Malix-Labs/NixOS_WinPE/.debug/hwreplica/replica-eval.nix`; 2) `AUTORUN_FILE=$PWD/autorun-pause.cmd WIM_FILE=$PWD/boot-n21.wim BIOS=$(readlink -f .debug/bios-result) bash .debug/hwreplica/stage2.sh`; 3) monitor socat screendumps; look at the death screendump; the `-SILENT` passthrough in the autorun needs `set FLASH_RC=` hand-traced in the pause autorun — keep local manual logs as the single source of truth.
- User reboot protocol unchanged: user reboot is ONLY requested when a healthy local replica gets the flasher to run AND the autorun branch output leaves the -148 golden diagnosis of H2OFFT-W.

**User wishes logged this session (rounds 14→21):**
