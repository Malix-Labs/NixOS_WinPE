...
## 0.6 HANDOFF STATE (2026-09-13 evening, PRE-COMPACTION) — READ FIRST

**User decision (binding): minimal unofficial modification, official binaries/tools only. The H2OFFT-W manifest-patch route (patch-manifest.py) was REVERTED. Current route: official `FWUpdLcl.exe` CLI flasher (no VC90/SxS deps, verified via objdump imports) + grafting the OFFICIAL x86 SystemCompatible assembly from Microsoft's own ESD into the WinPE's WinSxS.**

UNCOMMITTED (commit after QEMU validation):

- `pkgs/lenovo-legion-bios/strip-manifest.py` (NEW): zeroes the RT_MANIFEST resource-directory entry counts (pefile; the type-24 subdirectory header at root+rel(entry.OffsetToData&0x7FFFFFFF), counts at +12/+14). Reason: the ESD WinPE cannot create activation contexts for 32-bit apps — EVERY manifest-bearing binary dies with ERROR_SXS_CANT_GEN_ACTCTX regardless of manifest content (vendor deps / trimmed private assemblies / dependency-free+identity / NO manifest at all — all fail identically, QEMU + hardware agree).
- `pkgs/lenovo-legion-bios/default.nix`: manifest patching REVERTED (vendor-pristine flat layout = the SFX's own arrangement); platform.ini hardening KEPT (Silent=1/Confirm=0, AC+Platform checks off, Log_file on, RETURN_SUCCESSFUL 0,0); installCheck now asserts per-PE `wrestool --list | grep -qE "type=24 --name"` must be ABSENT (beware: plain `grep type=24` false-matches the benign `type=240` resource in mfc90u.dll!); nativeBuildInputs += python3.withPackages pefile; installCheck += icoutils.
- `profiles/lenovo-legion-15ach6h.nix`: entryPoint = `FWUpdLcl.exe`, silentFlags = [] (flag discovery pending — FWUpdLcl with no args prints usage; adjust after the first QEMU run).
- FWUpdLcl imports ONLY system DLLs (SETUPAPI/KERNEL32/ADVAPI32 — objdump-verified) → no SxS exposure. H2OFFT-W.exe (GUI flasher) needs VC90 CRT/MFC SxS → unusable without VC90 grafting → not the entry point.

**ROOT CAUSE (this round's breakthrough):** Win11 WoW64 (32-bit) processes require the x86 `Microsoft.Windows.SystemCompatible` assembly for their DEFAULT activation context. The ESD WinPE ships the amd64 one (microsoft.windows.systemcompatible 6.0.26100.1, token 6595b64144ccf1df — 2 entries) but ZERO x86 copies (its WinSxS x86/wow64 population = servicing-stack/boot-environment leftovers only). Hence every 32-bit process dies at default-context creation, regardless of its own manifest. The amd64 SystemCompatible manifest is manifest-ONLY (no <file> payload) with one REQUIRED dep: `Microsoft.Windows.IsolationAutomation` 1.0.0.0 (x86 also absent from the guest — graft it too; verify its own content for further deps).

**NEXT-STEPS RUNBOOK (exact):**

1. ESD re-download: started ~19:47 (background `nix build .#packages.x86_64-linux.winpe-image.src -o .debug/hwcheck/esd`, log `.debug/hwcheck/esd-build.log`). The OLD store path (`vf1mfb8n...esd`) held a TRUNCATED 3.09GB partial (wiminfo: "unexpectedly reached end of file") — never trust it; wait for completion (~15 min for 4GB) and verify with `wiminfo` (must print Image Count 5-ish). If the background client died, just re-run the nix build (fetchurl restarts from zero).
2. Find the OS image: `wiminfo ESD` → images 1,2 = PE environments; 3/4 = Home/Pro OS images. `wimdir ESD <n> | grep -i systemcompatible` per image → use one that lists BOTH `WinSxS/x86_microsoft.windows.systemcompatible*` AND `WinSxS/Manifests/x86_microsoft.windows.systemcompatible*`.
3. Extract to `.debug/sxs/`: `wimextract ESD <n> "Windows/WinSxS/x86_microsoft.windows.systemcompatible*" "Windows/WinSxS/Manifests/x86_microsoft.windows.systemcompatible*" "Windows/WinSxS/Manifests/x86_microsoft.windows.isolationautomation*" --dest-dir=.debug/sxs` (the SystemCompatible manifest's REQUIRED dep is IsolationAutomation 1.0.0.0 x86 — the guest lacks it; check the extracted IsolationAutomation manifest content for further deps; both are expected manifest-only, no payload files).
4. Graft into the guest WIM (`.debug/hwcheck/boot.wim` — the hardware's own copy): build the CURRENT `winpe-flash.startnetScript` + `winpeshlIni` (nix build .#packages.x86_64-linux.winpe-flash.startnetScript etc.), then `wimupdate boot.wim 1 --command="add <store> /Windows/WinSxS/..."` per grafted path AND re-add the current startnet/winpeshl (the hwcheck copy holds the pre-diagnostic startnet).
5. Re-stage the replica ESP: mkfs p4 + result-winpe Boot/EFI + the updated boot.wim + autorun (jadygaqq is STALE — build the current profile autorun via `.debug/hwreplica/autorun-eval.nix` pattern with -f/--impure, or sed jadygaqq: H2OFFT-W.exe→FWUpdLcl.exe + flags→"") + firmware/GKCN65WW from the CURRENT bios package.
6. QEMU replica boot (recipe §0.5 bottom) → collect `payload.out`/`autorun.log`. EXPECT: NO SxS error; FWUpdLcl prints usage (no args) or an update attempt against OVMF. If usage: read the flag list from the output → set profile silentFlags → re-run locally until correct.
7. THEN: full `nix flake check -L` → commit main (no tag) → push → dotfiles bump → user: `nh os switch` + reboot (the REAL flash — fans ~2 min) → `cat /sys/class/dmi/id/bios_version` = GKCN65WW.

CONTEXT:

- QEMU replica harness + staging recipe: §0.5 bottom; replica artifacts `.debug/hwreplica/` (disk.img 4-partition, VARS.fd, dumps r2..r10, autorun-fwupdlcl.cmd = the hand-patched FWUpdLcl autorun, autorun-eval.nix = the profile-autorun eval file).
- Current bios package (all installCheck asserts green): `/nix/store/a4p3c8aj...-bios-65` — but it PREDATES the FWUpdLcl pivot; rebuild after the profile change.
- The guest WinPE constraints: 32-bit processes need x86 SystemCompatible in WinSxS (default context); amd64 SystemCompatible verified present in the guest; x86 absent (the gap). H2OFFT-W additionally needs VC90 CRT/MFC SxS — NOT grafted (WinSxS dir-name hash algorithm undisclosed per auscitte/Wiswall research; brute-force MD5/SHA permutation search over 4550 known pairs found nothing) — FWUpdLcl sidesteps it.
- The ESD WinPE guest also LACKS x86 IsolationAutomation (grep: 5 amd64-ish entries, 0 x86).
- dotfiles lock: f86c33f (→ b8649c4). The user's current generation stages nkwdmpic (917259e-era: subdirs+trimmed manifests, UNPATCHED exe) — the round-5 SxS state. The FWUpdLcl pivot is NOT in the user's switch until committed + pushed + dotfiles bumped + `nh os switch`.
- The tag decision (v0.6.1) is still pending — only after the real-hardware flash succeeds.
