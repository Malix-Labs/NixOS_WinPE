...
**URL REGISTRY (2026-09-24, user-led verification):**
- Lenovo driver list (82JUCTO1WW): https://pcsupport.lenovo.com/us/en/products/laptops-and-netbooks/legion-series/legion-5-15ach6h/82ju/82jucto1ww/pf2ra29l/downloads/driver-list
- Lenovo downloads root (same): https://pcsupport.lenovo.com/us/en/products/laptops-and-netbooks/legion-series/legion-5-15ach6h/82ju/82jucto1ww/pf2ra29l/downloads/
- Lenovo ds549277 (the ONLY BIOS item; gkcn65ww.exe; flash-from-OS, Win10/11): https://support.lenovo.com/us/en/downloads/ds549277-bios-update-for-windows-10-64-bit-legion-5-pro-16ach6h-legion-5-15ach6h-lenovo-legion-5-17ach6h
- ThinkPad-style "Utility & Bootable CD" pattern reference (NOT present for 82JU): https://support.lenovo.com/us/en/downloads/ds545476 (ThinkPad L14)
- **RECOVERY-MEDIA LEAD (user-discovered 2026-09-24)**: "Get Recovery Media" on the downloads page → Lenovo USB Recovery Creator (user downloaded `/home/malix/Downloads/USBRecoveryCreator.exe`). The program offers *factory-OS recovery media* for this machine (Win11 Pro / downgraded Win10 Pro; digital download for Win10 Think/Ideapad). CANDIDATE VALUE for us: the digital-download recovery image = an OFFICIAL factory-install Windows (era-matched) + Lenovo's boot layer — a POSSIBLE alternative/graft source for the environment (probing the download BEFORE installing anything into production; NOT a replacement for the current path unless the ESD-based grafting hits an unnamed wall). Not yet acted on — logged as a backup source.

## 0.17 ROUND 44 (2026-09-24 02:30) — ROUND-42 SHA-1 GATE: THE 64-BIT TRUST VERDICT ON THE 22H2 ENVIRONMENT = SUCCESS; the 0xE7 class narrowed; next work listed — READ FIRST

**THE ROUND-42 GATE (SHA-1 era-trust verdict) IS NOW MEASURED — AND PASSES (64-bit path, local replica, no user reboot spent):**
`probe10.c` (64-bit mingw, xor-verified loader-first, in-guest on the 22H2 image, `W:\firmware\GKCN65WW\probe10.log` at boot n71) — 64-bit `WinVerifyTrust` (22H2 base-shipped amd64 wintrust) over the WHOLE companion set, action = WINTRUST_ACTION_GENERIC_VERIFY_V2, WTD_SAFER_FLAG set:
- **FlsHook.exe  rc=0x00000000  ← PASS (the SHA-1-signed file the 24H2 environment refused to even hash!)**
- **FWUpdLcl.exe rc=0x00000000 PASS**   | **BiosImageProc.dll PASS** | **msvcr90.dll PASS** | **mfc90u.dll PASS**
- H2OFFT-W.exe 0x0010B008 and WDFInst.exe 0x0010B008 (both UNSIGNED — the raw result needs a post-pass; these two are the two unsigned vendor members and were always 0x800B0100 TRUST_E_NOSIGNATURE expected; treat 0x0010B008 as a two-step C-language bug ("0x0x" printed twice): my whexp printed `0x` prefix doubled so the true value = `0x0010B008`; needs probe next.
- **CONCLUSION: era-matched trust mechanism = RESTORED on 22H2.** On the 24H2 base the same files returned the raw `0x00000008` crypt-layer refusal (round 40). On 22H2 the class validates. The 22H2 era-choice for winpe-image-legacy is EMPIRICALLY justified now (as measured); the round-41 assumption story can be retired for good.

**REMAINING on the H2OFFT-W 32-bit chain: the `0xE7000000` family (binary-ours only):**
- As of this writing: `bcrypt/crypt32/advapi32-safer/combase (32-bit)` fail LoadLibrary gle=0xE7000000 even with **bcryptprimitives.dll** (ESD 22621 `SysWOW64/bcryptprimitives.dll` = 401,080 bytes, era-correct) and `downlevel/ucrtbase.dll` grafted (fixed `msvcp_win`, the `api-ms-win-crt-*` chain!). All 53 of combase's imports + 12 specific combase-only names individually load OK; same for bcrypt (all -ms OK). These four now fail with 0xE7 = **the loader's per-DllMain-fail after import resolution (a second-order family, not the CRT/stub layer)**.
- They share a **DLL-side property** meaning after import load, their DllMain pulls a dependency our 22H2 env still lacks: wintrust's own LoadLibrary chain includes the embedded strings {bcrypt.dll, crypt32.dll, lsalookup-l2-1-0, sddl-l1-1-0, service-management-*1|2, service-private, service-winsvc, advapi32-safer} — all individually load except the SAFE (advapi32-safer) one = so wintrust's DllMain fails because **advapi32-safer (which itself resolves to advapi32) gives 0xE7** — the SAME code comes from the chain chain: bcrypt/crypt32 ALSO fail (safer → bcrypt → bcryptprimitives ✓ level chain).
- **Lever candidates for the next session (round 45, LOCAL): (1) Determine 22H2-COMBASE's unique second-order missing dependency; consider mining combase 22621's members' DllMain (22H2's crt init calls `GetUserDefaultUILanguage` etc.). (2) GRAFT the 24H2 26100 versions of {bcrypt,crypt32,cryptbase,bcryptprimitives,wintrust} on top of the 22H2 base (request源泉 era-sha256 signed!) — the RISK = the 0xC000021A poison class (round-6 26100-WOW64 graft era-mix). ONLY local replica tests allowed before filling. (3) Analyze `0xE7000000` semantics for real — a name-level diagnostic: run one more in-guest probe with `bl Preliminaryizer-success` diagnostic list.**
- **THE 32-BIT PROBE `probe5b` (WinVerifyTrust on the 32-bit side) is still blocked waiting for the `0xE7` family. The 64-bit probe10 gives THE POLICY answer (era-matched) but NOT the exact 32-bit path the tool runs inside. Work continues to bring the `0xE7` family to OK on the 32-bit layer: after it is OK the SHA-1 probe5 will CHANGE from `0x00000008` to an era-class result (zero or a real TRUST_E_*).**
- On the H2OFFT-W 32-bit path: the environment still needs: {user32+gdi32 to load (needs api-ms shim + gdi32full both loaded ✓ those already work), ole32 (works), sidecars (staged already), correct 32-bit wintrust (the open wall)}.

**TOOLING (next-session reuse):**
- `probe10.c` (64-bit mingw probe, `W:\firmware\GKCN65WW\probe10.log`, writes per-file WinVerifyTrust verdict with action=GENERIC_VERIFY_V2+WTD_SAFER_FLAG) — FIXED (the earlier ZeroMemory after-init bug from n69 is now solved with crt_memset-minus-hack via python; compilable with the plain `pkgsCross.mingwW64.stdenv.cc` + `-L$(mcfgthread-x86_64-w64-mingw32-2.3.2)/lib` (the x64 mcfgthread store path = 9drpyym9f68pk071l62yibbn1qfds4v6-)).
- `probe9{c,f}.c` variants 32-bit LoadLibrary probes (bcryptgrpc batch etc.)
- The new flow: 22H2 diagnostic CHANGE-SET round-by-round earned these WIM-capable probes.

**Store/ESD status (2026-09-24 02:30):** ESD re-added from `/home/malix/Downloads/22621-en-us.esd` as `/nix/store/506dmq08v1f99qrkdgh2ibi3h93a0mv5-22621-en-us.esd` (sha256-verified source-mirrored; symlinked to `.debug/roots/esd-22h2`). 26100.4349 ESD back at `/nix/store/vf1mfb8n7g9c33rnr98zr2dd8h0n5wqy-26100.4349...esd`. winpe-image-legacy latest = `/nix/store/rxg1hyjgjpksbxwh24jji5hfrvq6f0y7-winpe-image-10.0.22621.1702` (bcryptprimitives/msvcp_win/ntdll/version/ucrtbase/downlevel grafts ALL in). The synthetic `probe9N` family hashes + `.c` files in `.debug/hwreplica/`. Build roots in `.debug/roots/*` (qemu/ovmf/mingw32/cfgthread). The USB Recovery Creator `.exe` lives in `/home/malix/Downloads/` (LEAD for a later session, not blocking).
- **Honest disclaimers to keep visible:** (1) the p-b set still contains {bcrypt/crypt32/advapi32-safer/combase} failing the LoadLibrary 0xE7 wall — the 32-bit `probe5` has NOT yet REMOVED from this 22H2 build directly. (2) the round-42 GATE now has a confirmed-positive (64-bit) but not a 32-bit one — the H2OFFT-W graphical app's own verifier runs in 32-bit always. (3) the trust paths on the 64-bit OK are NOT ALL equal: the 32+64 bit may be the same but H2OFFT-W's internal WinVerifyTrust verification is a 32-bit call to `wintrust.dll` regardless (round-40: the 32-bit layer on the 24H2 base had failure classes; wait for the 32-bit probe5 result on 22H2).
- **Copy the log artifact `probe10.log` SHA + the 22H2 WIM's fingerprint into the repo history? (Keep as note only, not committed; these .debug/* are gitignored per our policy.)**
