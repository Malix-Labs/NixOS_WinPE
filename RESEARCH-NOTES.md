...
## 0.6 HANDOFF STATE (2026-09-15 ~22:00, 32-BIT PATH SOLVED + PRODUCTIONIZED) — READ FIRST

**User decision (binding): minimal unofficial modification, official binaries/tools only; update scripts must not break (or fail with named errors) and everything must be checkable. UNCOMMITTED on main (a06c89e): flake.nix + pkgs/winpe-image + pkgs/winpe-flash + profiles/lenovo-legion-15ach6h.nix + this file. `nix flake check -L` = FULLY GREEN. Dotfiles still f86c33f. No tag.**
