...
**LOCAL REPLICATION (byte-exact): rebuilt a boot.wim replica via cp of the built store file + wimupdate adds of winpeshl.ini + startnet.cmd → 593,462,906 bytes = EXACTLY the size of the deployed file on the user's partition. QEMU boot of this replica (q35, OVMF.fd = pkgs.OVMF.fd, TCG, -no-reboot): boot proceeds through BdsDxe → winload → TEXT console (screendump = dark screen + bright top band = text console, NOT the uniform-blue graphical recovery UI). So under QEMU the same bytes run our flow.**

**ENVIRONMENT FACTS from the image (offline inspection):** stock HKLM\SYSTEM\Setup has "CmdLine"="winpeshl.exe", SetupType=1, SystemSetupInProgress=1, FactoryPreInstallInProgress=1 — kernel-appointed shell = winpeshl.exe, which SHOULD read our ini. The image's EFI tree: EFI/Boot/bootx64.efi + EFI/Microsoft/boot/{resources,fonts,cipolicies,cdboot*.efi,memtest.efi} but NO BCD and NO bootmgfw.efi in EFI/Microsoft/boot; the only BCD = legacy boot/bcd (PCAT/BIOS store: "Windows Setup" loader on \sources\boot.wim via \boot\boot.sdi, path \windows\system32\boot\winload.exe). Bootmgr's fallback BCD order reportedly: \EFI\Microsoft\Boot\BCD → \boot\bcd → \bcd (QEMU's BdsDxe loads our disk fine, presumably via the \boot\bcd fallback; REAL hardware passed round 9 with the same tree, so the missing EFI-BCD is probably NOT the round-10 trigger).

**OPEN QUESTION (the load-bearing one): what makes the WinRE-based image skip our [LaunchApps] chain on real hardware but not under QEMU? Candidates: (a) the Legion firmware's boot path differs (e.g. boots via the machine's own ESP bootmgfw+real BCD with Windows-Recovery-style entry flags → WinRE-mode kernel behavior → recenv-first shell policy), (b) winpeshl.exe acts on embedded/SxS shell-decision state not visible offline, (c) fast-boot/one-shot BootNext semantics interacting with the image's recovery state. Round 9 (old setup-PE boot.wim, 422MB) DID reach startnet+autorun on the same machine, so the machine-side boot chain works for a WinPE-mode image.**

**NEXT-STEP PLAN (in order):**
1. Ask the user to break out of the blue menu: Troubleshoot → Advanced options → Command Prompt (WinRE's own shell), then photograph the output of `type X:\Windows\System32\winpeshl.ini` and `reg query HKLM\SYSTEM\Setup` (CmdLine/SetupType values as seen by the booted kernel) — key evidence of which shell decision state the real boot actually used.
2. If the booted image shows our ini but the recovery UI ran anyway: investigate WinRE-mode shell ordering (RecEnv vs [LaunchApps]); consider moving/renaming X:\sources\recovery\recenv.exe OUT of the image at build time (fallback-to-cmd test) as the pragmatic fix — with QEMU screendump + real-hardware validation before relying on it.
3. Do NOT attempt offline hive edits (established negative: hive rewrite breaks the boot).
